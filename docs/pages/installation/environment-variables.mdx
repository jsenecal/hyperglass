...
| `HYPERGLASS_WORKERS`        | number  | auto              | Number of uvicorn worker processes. When unset, hyperglass auto-detects the cgroup/affinity-allowed CPU count and uses `min(cpu_count * 2, 8)` (or `1` when `HYPERGLASS_DEBUG` is `true`). Set explicitly when running on a host whose CPU count differs from the container's allocation, or to pin workers below the auto-cap. |
